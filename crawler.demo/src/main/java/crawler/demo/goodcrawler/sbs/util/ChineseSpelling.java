...
private static String[] pystr = new String[] { "a", "ai", "an", "ang",
	...
	private int getChsAscii(String chs){
		...
		try {
			...
			if (bytes.length == 2) {   //chinese
				int hightByte = 256 + bytes[0];
				int lowByte = 256 + bytes[1];
				asc = (256 * hightByte + lowByte) - 256*256;
			}
		} catch (Exception e) {
			System.err.println("ERROR : ChinesesSpelling.class-getChsAscii(String chs) " + e );
		}
		return asc;
	}
	...
	public static void main(String[] args) {
		...
		finder.setResource("ÖÐÎÄ×Ö·ûsdlkfjsdlfj");
	//	System.out.println(finder.getSpelling());
	//	System.out.println(finder.getSelling("Ó¢ÎÄ×Ö·ûEng"));
	//	System.out.println(finder.convert("ÄãºÃÂð"));
	}
}
