...
public class UrlResolver {
	...
	private static Url parseUrl(final String spec){
		...
		final int semicoloIndex = indexOf(spec, ';', startIndex, endIndex);
		
		if (semicoloIndex >= 0) {
			if ((locationStartIndex >= 0) && (locationEndIndex < 0)) {
				locationEndIndex = semicoloIndex;
				startIndex = semicoloIndex;
			}
			url.parameters_ = spec.substring(semicoloIndex + 1,endIndex);
			endIndex = semicoloIndex;
